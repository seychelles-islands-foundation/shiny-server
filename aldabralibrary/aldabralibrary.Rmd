...
items <- reactive({
		input$add
	validate(
		need(db_location, "The specified location for the database file containing the library records is not valid. Go to advanced > 'Location of the database file' and tell me where I can read an existing database or create a new one.")
	)
...
renderDataTable({
	items() %>%
		dplyr::mutate(library_number = paste(lcc, cutter_author, cutter_title, sep = ".")) %>%
