---
title: "SIF Aldabra tidal predictions v0.1 (demo)"
resource_files:
...
year_pred <- reactive({
  paste0("./data/processed/tide_pred/loc_", 
         as.integer(input$Loc),
         "/", 
         as.character(lubridate::year(input$DateRange[1])),
         ".rds") %>%
    readRDS()
})

# filter water height for a time interval
read_height <- function(dr){
  year_pred() %>%
    dplyr::filter(time >= dr[1],
                  time <= dr[2]) %>%
    dplyr::mutate(time = as.POSIXct(as.POSIXlt(time, tz = "Indian/Mahe")))
}
...
current_tide <- reactive({
  h <- read_height(c(c_t - 7*3600, c_t + 7*3600))
  current_h <- dplyr::slice(h, round(nrow(h)/2))
  get_high_low_table(h) %>%
